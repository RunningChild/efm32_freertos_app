...
static void DisableTransmitter(UARTDRV_Handle_t handle)
{
  ...
  else if (handle->type == uartdrvUartTypeLeuart)
  {
    ...
    //_by_pbh add leuart用低频时钟时，需要考虑低频域同步问题
    // Wait for TX to be disabled
    while (handle->peripheral.leuart->STATUS & LEUART_STATUS_TXENS);
  }
}
 ...
 ******************************************************************************/
#if (EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE)//_by_pbh
...
//    if(LOG_USART_DEVICE != handle->peripheral.leuart)
//    {
//        LOG(LEVEL_DEBUG, "txDmaCh=%d", handle->txDmaCh);
//    }
...
//    if(LOG_USART_DEVICE != handle->peripheral.leuart)
//    {
//        LOG(LEVEL_DEBUG, "rxDmaCh=%d", handle->rxDmaCh);
//    }
...
/*当MCU工作在EM2模式时，为了执行DMA传送，必须将DMA控制器上电。
对于读操作，如果LEUARTn_CTRL中的RXDMAWU被置位，则上电过程是自动完成的；
对于写操作，如果LEUARTn_CTRL中的TXDMAWU被置位，该上电过程也是自动完成的。
为确保DMA控制器在低能耗模式仍能与LEUART进行数据传送，必须正确配置这些位。
在EM2模式，DMA使用HFRCO运行，所以该高频振荡器也被自动唤醒和关断。
在使用DMA处理LEUART之前，还必须正确使能和配置DMA。 */
#if 1//_by_pbh
  LEUART_TxDmaInEM2Enable(handle->peripheral.leuart, true);
  LEUART_RxDmaInEM2Enable(handle->peripheral.leuart, true);

  LEUART_TxDelayEnable(handle->peripheral.leuart, true);
#endif
...
//  LOG(LEVEL_DEBUG, "free txDmaCh=%d, rxDmaCh=%d", handle->txDmaCh, handle->rxDmaCh);
//  DMADRV_DeInit();//_by_pbh add
                        ...
                        UARTDRV_Callback_t callback)
{
  ...
  if (retVal != ECODE_EMDRV_UARTDRV_OK)
  {
//    LOG(LEVEL_FORCE, "[UARTDRV_Receive] rxQueue full.");
    return retVal;
  }
  if (!(handle->rxDmaActive))
  {
    StartReceiveDma(handle, queueBuffer);
    EnableReceiver(handle);//_by_pbh 防止上电立即接收到数据导致异常
#if (EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE)
    if (handle->fcType != uartdrvFlowControlHwUart)
    {
      handle->fcSelfState = uartdrvFlowControlOn;
...
/***************************************************************************//**
 * @brief uartdrv_get_transfered_len
 ******************************************************************************/
bool uartdrv_get_transfered_len(UARTDRV_Handle_t handle)
{
    ...
    {
        GetTailBuffer(handle->rxQueue, &rxBuffer);
        StartReceiveDma(handle, rxBuffer);
    }
    else
    {
#if EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE
    ...
    //重新使能
    DMA_ChannelEnable(handle->rxDmaCh, true);
...
/***************************************************************************//**
 * @brief uartdrv_rx_channel_enable
 ******************************************************************************/
bool uartdrv_rx_channel_enable(UARTDRV_Handle_t handle, bool enable)
{
    if (NULL == handle)
    {
        return false;
    ...
    DMA_ChannelEnable(handle->rxDmaCh, enable);

    if(false == enable)
    {
        uint32_t itemsRemaining;
        DMADRV_TransferRemainingCount(handle->rxDmaCh, (int*)(&itemsRemaining));
    }
