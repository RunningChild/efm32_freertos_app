...
/********************** Configuration of Platform.h ****************************/

//MCU空闲状态进入低功耗使能位
#define MCU_LOWER_POWER_ENABLE                      (1)//(1)
...
/* Energy saving modes */
#define configUSE_TICKLESS_IDLE                     (MCU_LOWER_POWER_ENABLE)
...
#define configUSE_SLEEP_MODE_IN_IDLE                (0 == MCU_LOWER_POWER_ENABLE)
...
#define configTICK_RATE_HZ    ( 250 )// must >=100, 被1000所整除
...
#define configTOTAL_HEAP_SIZE                     (( size_t )(20000*3))
...
#define configTIMER_QUEUE_LENGTH                  ( 45 )//EMDRV_RTCDRV_NUM_TIMERS
#define configTIMER_TASK_STACK_DEPTH              ( configMINIMAL_STACK_SIZE + 500 )

//_by_pbh
//需要调用 free rtos api函数的ISR，使用中断优先级范围：2 到 7 //(5 到 7)
//不会被free rtos的内核延迟并且可嵌套，但是不可调用free rtos api函数，使用中断优先级范围：0 到 1 //(0 到 4)
/* Interrupt nesting behaviour configuration. */
#define configKERNEL_INTERRUPT_PRIORITY           ( 255 ) //EFM32：[111]:7
//#define configMAX_SYSCALL_INTERRUPT_PRIORITY      ( 191 ) /* equivalent to 0xa0, or priority 5. */ //EFM32：[101]:5
#define configMAX_SYSCALL_INTERRUPT_PRIORITY      ( 95 ) /* equivalent to 0x04, or priority 2. */ //EFM32：[010]:2
...
#define configCPU_CLOCK_HZ                        (( unsigned long ) configCPU_CLOCK_HZ_DEFINE)
...
#else
#define configUSE_IDLE_HOOK  ( 1 ) //_by_pbh : 1
#endif
