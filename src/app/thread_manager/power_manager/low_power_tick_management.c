 ...
 * @brief EFM32_sleep
 ******************************************************************************/
__attribute__(( weak )) void EFM32_sleep(void)
{
    SLEEP_Sleep();
}
...
{
//    LOG(LEVEL_FORCE, "IdleHook");

    //configUSE_TICKLESS_IDLE == 1时，无须再次调用SLEEP_Sleep进入低功耗。
//    SLEEP_Sleep();
}
#endif
...
#if (OS_SYSTICK_RTC == OS_SYSTICK_SOURCE)
    #warning "os systick use rtc"
...
#elif (OS_SYSTICK_BURTC == OS_SYSTICK_SOURCE)
    #warning "os systick use burtc"
...
void vPortSetupTimerInterrupt(void)
{
  ...
  burtcInit.clkDiv      = burtcClkDiv_2;     /* Choose 32768HZ/2 clock frequency */
#endif
...
#else
    #error "Unknown or unsupported os systick source."
#endif
